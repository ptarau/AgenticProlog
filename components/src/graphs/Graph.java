...
public class Graph extends ObjectDict {
  ...
  */
  ...
  public void setSelectFilter(String sfilter) {
    if("key".equals(sfilter))
      filter=KEY;
    else if("data".equals(sfilter))
      filter=DATA;
    else if("id".equals(sfilter))
      filter=ID;
    else if("rank".equals(sfilter))
      filter=RANK;
    else {
      filter=ATTR;
  ...
  */
  ...
  /**
   * Depth-first visit of a graph, with callbacks to a GraphVisitor
   */
  ...
  /**
   * recusive visit of unvisited vertices
   */
