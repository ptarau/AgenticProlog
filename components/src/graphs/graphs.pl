new_dict(D):-
  new_java_class('compat.ObjectDict',C),
...
new_ranked_graph(G):-
  new_java_class('graphs.RankedGraph',C),
  new_java_object(C,void,G).
...
term_to_hyper(T,C):-call_java_class_method('graphs.Cat',fromTerm(T),C).

term_to_cat(T,C):-call_java_class_method('graphs.Cat',termTree(T),C).

term_to_cat(T,FType,CType,C):-call_java_class_method('graphs.Cat',termTree(T,FType,CType),C).

new_small_cat(C):-
  new_java_class('graphs.SmallCat',Cat),
  new_java_object(Cat,void,C).
...
to_qdot(Cat,FileName):-to_dot(Cat,FileName,0,0).

to_dot(Cat,FileName):-to_dot(Cat,FileName,1,1).

to_vdot(Cat,FileName):-to_dot(Cat,FileName,2,2).

to_ldot(Cat,FileName):-to_dot(Cat,FileName,3,1).


%% to_dot(Cat,FileName,VerboseVs,VerboseEs): writes out a Cat to a dot file for graphviz visualization
to_dot(Cat,FileName,VerboseVs,VerboseEs):-
  invoke_java_method(Cat,to_dot(FileName,VerboseVs,VerboseEs),_).
  
/* example:
?- rancat(C),new_cat(N),(cat_to_prolog(C,N:P),P,fail;true),cat_to_prolog(N,NP).
?- rancat(C),cat_term_of(C,T),println(T),fail;nl.
*/

%% cat_to_terms(Cat,Terms): converts a Cat graph to a list of (attributed) Terms of the form vertex-edges
cat_to_terms(C,Ts):-findall(T,cat_term_of(C,T),Ts).

%% cat_term_of(Cat,V-[Rank,Attr,Value,EdgesWithProps]): iterates over vertices V of a Cat graph
cat_term_of(C,V-[R,X,A,Props]):-
  prop_of(C,V,X,A),
  get_rank(C,V,R),
  findall([T,M,D],morphism_of(C,V, T,M,D),Props).
...
pp_cat(C):-
    ...
    portray_clause(VV:XX=RR),
  fail.
pp_cat(C):-
    morphism_of(C,F,T,M,D),
    maplist(unwrap_object,[F,T,M,D],[FF,TT,MM,DD]),
    portray_clause(FF=>(TT:MM=DD)),
  fail.
pp_cat(_C):-nl.

%unwrap_object(O,S):-nonvar(O),functor(O,'$object',1),!,object_to_string(O,S).

unwrap_object(O,S):-other_object(O),!,object_to_string(O,S).
unwrap_object(T,T).


% TESTS for Graph, Cat etc.
 
rgtest:-
  call_java_class_method('graphs.RankedGraph',rgtest,_).

% srgtest:-call_java_class_method('graphs.RankedGraph',srgtest,_).
  
gtest:-
  call_java_class_method('graphs.Graph',gtest,_).
    
hgtest:-
  println('JAVA'),
  call_java_class_method('graphs.RankedGraph',hgtest,_),
  nl,
  println('PROLOG'),
  new_ranked_graph(G),
...
% ctest:-call_java_class_method('graphs.Cat',ctest,_).
...
scattest:-
  call_java_class_method('graphs.SmallCat',sctest,_).  

gv_show:-gv_show('gr.gv').
  
gv_show(File):-system([open,File],_).
  
gv_test:-
  traceln('!!! make sure you have graphviz from www.graphviz.org installed !!!'),
  rancat(C0),to_qdot(C0,'gr.gv'),gv_show,
  foreach(
    for(I,1,5),(traceln(vdot(I)),sleep(5),rancat(C),to_vdot(C,'gr.gv'),gv_show)
  ),
  foreach(
    for(I,1,5),(traceln(dot(I)),sleep(5),rancat(C),to_dot(C,'gr.gv'),gv_show)
  ),
  foreach(
    for(I,1,5),(traceln(qdot(I)),sleep(5),rancat(C),to_qdot(C,'gr.gv'),gv_show)
  ),
  println(done).
...
unpair2list(F,Z,[X,Y]):-call(F,Z,X,Y).

unpair2list1(F,Z,[X,Y]):-call(F,Z,X-Y).

%% show_unpair(F,N): if F is unpair(Z,X,Y) is a wf unpairing function in draws the recursive unfolding graph of N
show_unpair(F,N):-show_as_cat(unpair2list(F),0,1, 1,N).

%% show_unpair1(F,N): if F is unpair(Z,X-Y) is a wf unpairing function in draws the recursive unfolding graph of N
show_unpair1(F,N):-show_as_cat(unpair2list1(F),0,1, 1,N).

%% show_hfun(F,N): if F is a wf N->[N] function in draws the recursive unfolding graph of N with nordered edges
show_hfun(F,N):-show_as_cat(F,0,1, 0,N).

%% show_hset(F,N): if F is a wf N->[N] function in draws the recursive unfolding graph of N withn unordered edges
show_hset(F,N):-show_as_cat(F,0,0, 0,N).

show_as_cat(F,ShowVs,ShowEs,Stopper,N):-
  unfold_to_cat(F,Stopper,N,C),
  to_dot(C,'gr.gv',ShowVs,ShowEs),
  gv_show.
    
unfold_to_cat(F,Stopper,N, C):-  
  new_cat(C),
  (add_fs_to_cat(N,Stopper,F,C),fail;true).
  
add_fs_to_cat(N,Stopper,_,C):-N=<Stopper,!,add_vertex(C,N,N).
add_fs_to_cat(N,Stopper,F,C):-
  call(F,N,Ns),
  set_prop(C,N,N,N),
  nth0(I,Ns,M),
  set_morphism(C,N,M,I,I),
  add_fs_to_cat(M,Stopper,F,C),
  fail.
