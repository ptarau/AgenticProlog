...
public class RankedGraph extends Graph {
  ...
  public RankedGraph(BigDecimal D,BigDecimal E,int maxIter){
    this(D.doubleValue(),E.doubleValue(),maxIter);
  }
  
  public RankedGraph(int size,BigDecimal D,BigDecimal E,int maxIter){
    this(D.doubleValue(),E.doubleValue(),maxIter);
    if(size<0)
      size=0;
  }
   ...
   */
  private final double getRank0(Object v) {
    RankedData rd=(RankedData)super.vertexData(v);
    return rd.rank;
  }
  
  public BigDecimal getRank(Object v) {
    return new BigDecimal(getRank0(v));
  }
   ...
   */
  public void setRank(Object v,BigDecimal rank) {
    double r=rank.doubleValue();
    setRank0(v,r);
  }
  
  private final void setRank0(Object v,double rank) {
    RankedData rd=(RankedData)super.vertexData(v);
    rd.rank=rank;
  }
  ...
  */
  ...
  public boolean graphRankerStep(boolean classic) {
    ...
    while(Vs.hasNext()) {
      Object V=Vs.next();
      double r0=getRank0(V);
      ObjectIterator Ts=classic?inIterator(V):outIterator(V);
      double sum=0.0;
      while(Ts.hasNext()) {
        Object T=Ts.next();
        double t=getRank0(T)/(classic?outDegree(T):inDegree(T));
        sum+=t;
      }
      double r=(1-d)+d*sum;
      setRank0(V,r);
  ...
  public RankedKey[] getRankedKeys() {
    ...
    while(Vs.hasNext()) {
      Object V=Vs.next();
      Ks[i++]=new RankedKey(V,getRank0(V));
    }
    Tools.sort(Ks);
    normalizeRanks(Ks);
  ...
  */
  ...
  public static String showInfo(RankedGraph RG) {
    ...
    s.append("\nhasCycles="+cyclic+",nb. of components="+c+",giant="+g
        +",gsize="+gs+" PR steps="+times+"\n");// +"\nGraph after ranking:\n"+RG.toString());
    for(int i=0;i<c;i++) {
