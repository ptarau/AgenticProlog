  ...
  */
  public Object pop() {
    if(isEmpty()||count!=top+1)
      return null; // not safe to use as a stack
  ...
  public void compact() {
    while(ftop>=0) {
      ...
      Entry e=syms[i];
      if(null!=e)
        Interact.errmes("error compacting dict at:"+ftop,new PrologException(
            "error in ObjectDict"));
      e=syms[top];
      syms[top--]=null;
      e.ordinal=i;
  ...
  public String info() {
    return "{count="+count+", entries="+entries.length+", top="+top+", ftop="
        +ftop+", get="+getCount+", add="+addCount+", update="+updateCount
        +", remove="+removeCount+"}";
  }
