...
% adds one
init_hash_db:-index('$hashed'(1,1,0)),index('$bb'(1,1,0)).

hash_put(GroundKey,Term):-
  term_hash(GroundKey,K),
  db_assert('$hash_db','$hashed'(K,GroundKey,Term)),
  add_hash_key(K).
...
hash_get0(K,GroundKey,Term):-
  db_clause('$hash_db','$hashed'(K,GroundKey,T),true),
  Term=T.

% removes first  
hash_rm(GroundKey,Term):-
  term_hash(GroundKey,K),
  db_retract1('$hash_db','$hashed'(K,GroundKey,Term)),
  del_hash_key(K).
...
hash_clear(GroundKey):-
  term_hash(GroundKey,K),
  db_retractall('$hash_db','$hashed'(K,GroundKey,_)),
  del_hash_key(K).
...
del_hash_key(K):- \+(db_clause('$hash_db','$hashed'(K,_,_),true)),!,rm('$hashkey',K),db_retractall('$hashkeys','$key'(K)).
