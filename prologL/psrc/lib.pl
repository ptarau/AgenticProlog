...
% real entry point: G_prolog_main run/3; search for: G_main
...
ground(X) :-
  arity(X,A),
  has_ground_args(0, A, X).

has_ground_args(A, A, _) :- !.
has_ground_args(A0, A, X) :-
  succ(A0,A1),
  arg(A1, X, X1),
  ground(X1),
  has_ground_args(A1, A, X).
